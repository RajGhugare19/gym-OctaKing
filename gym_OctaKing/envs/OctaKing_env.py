...
class OctaKingEnv(gym.Env):
    ...
    def check(self,move):
        ...
        to_row,to_col = self.getRowCol(to)
        frm_row,frm_col = self.getRowCol(frm)

        if self.done:
            print('Done flag is true')
            return 0

        if self.state[player,frm_row,frm_col] == 0:
            return 0
